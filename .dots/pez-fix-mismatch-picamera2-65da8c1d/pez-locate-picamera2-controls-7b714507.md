---
title: Locate picamera2 controls divergence
status: open
priority: 2
issue-type: task
created-at: "2026-02-05T21:25:22.588980+01:00"
blocks:
  - pez-repro-picamera2-controls-b9b7e2da
---

Context: tools/compare/locate_mismatch.py:1, tools/compare/unit_trace.py:1; cause: first divergence and CFG delta unknown; fix: run locate_mismatch+unit_trace and map diverging block/edge signatures; deps: pez-repro-picamera2-controls-b9b7e2da; verification: locate/unittrace artifacts for picamera2 controls written to /tmp.

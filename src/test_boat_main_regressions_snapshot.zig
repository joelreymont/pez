...
test "snapshot subprocess wait lock release 3.9" {
    try runSnapshot(@src(), "test/corpus/subprocess_wait_lock_release.3.9.pyc",
        \\[]u8
        \\  "import os
        \\import time
        \\class Proc:
...
test "snapshot subprocess run timeout handlers 3.9" {
    try runSnapshot(@src(), "test/corpus/subprocess_run_timeout_handlers.3.9.pyc",
        \\[]u8
        \\  "_mswindows = True
...
test "snapshot with conditional open as 3.9" {
    try runSnapshot(@src(), "test/corpus/with_conditional_open_as.3.9.pyc",
        \\[]u8
        \\  "_DEFAULT = object()

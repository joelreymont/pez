...
pub const Decompiler = struct {
    ...
    fn tryDecompileBoolOpInto(
        ...
        const first = try cond_sim.stack.popExpr();
        errdefer {
        ...
        while (block_idx < self.cfg.blocks.len) {
            ...
            switch (pattern) {
                .if_stmt => |p| {
                    ...
                    try self.processPartialBlock(&self.cfg.blocks[p.condition_block], &self.statements, self.allocator, &skip_first_store);
    ...
    fn decompileBlock(self: *Decompiler, block_id: u32) DecompileError!void {
        ...
        if (!needs_exc and seed.len == 0 and needs_predecessor) {
    ...
    fn handlePopTopStmt(
        ...
        if (sim.stack.len() == 0) {
            if (self.allowsEmptyPop(block)) return;
            return error.StackUnderflow;
        }
        const val = sim.stack.pop().?;
        switch (val) {

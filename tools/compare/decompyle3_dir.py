...
def run_decompyle3(decompyle3: Path, pyc: Path, timeout: int) -> tuple[int, str, str]:
    proc = subprocess.run(
        [str(decompyle3), str(pyc)],
...
def main() -> None:
    ...
    for pyc in sorted(orig_root.rglob("*.pyc")):
        ...
        rc, out, err = run_decompyle3(decompyle3, pyc, args.timeout)
        counts["total"] += 1
        if rc == 0:
            counts["ok"] += 1
